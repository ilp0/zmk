 ...
 * @return int 
 */
static int iqs5xx_write(const struct device *dev, const uint16_t addr, uint8_t *buf, uint8_t len) {
    LOG_ERR("\nwrite iqs5xx");
    const struct iqs5xx_data *data = dev->data;
    const struct iqs5xx_config *config = dev->config;
    // Write address
    int err = i2c_write(data->i2c, (const uint8_t*)&addr, sizeof(uint16_t), AZOTEQ_IQS5XX_ADDR);

    err |= i2c_write(data->i2c, (const uint8_t*)buf, len, AZOTEQ_IQS5XX_ADDR);
    LOG_ERR("\n%d", err);
...
DEVICE_DT_INST_DEFINE(0, iqs5xx_init, device_pm_control_nop, &iqs5xx_data, &iqs5xx_config, POST_KERNEL, 20, &iqs5xx_driver_api);