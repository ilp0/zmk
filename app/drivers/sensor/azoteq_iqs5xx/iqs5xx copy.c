...
static int iqs5xx_read_reg(const struct device *dev, uint16_t reg,
			      void *data, size_t length)
{
	uint8_t tp_data[44];

	uint8_t addr_buffer[2];
	addr_buffer[1] = reg & 0xFFU;
	addr_buffer[0] = reg >> 8U;

	i2c_write_read(dev, AZOTEQ_IQS5XX_ADDR,
			      addr_buffer, sizeof(addr_buffer),
			      &tp_data, 44);

	//i2c_burst_read(dev, AZOTEQ_IQS5XX_ADDR<<1, reg, &tp_data, 44);
	uint8_t 	i; 
    ...
    LOG_DBG("TRACKPAD ABS Y %hn", d.ui16AbsY);
...
static bool iqs5xx_init(const struct device *dev) {
	//TODO:: INIT ON VITUILLAAN. KATSO MCP23017 mallia. MÄÄRITÄ DRV_DATAT kuntoon
    LOG_DBG("\ntrackpad setup start\n");
    struct iqs5xx_dev_data *const data = dev->data;
	const struct iqs5xx_dev_config *cfg = dev->config;
    data->config.addr = device_get_binding(DT_PROP_);

	uint8_t     activeRefreshRate[2] = {0,8};
	uint8_t     idleRefreshRate[2] = {0,8};
	uint8_t		stop = 1;

	i2c_write(dev->data, &activeRefreshRate[0], 2, ActiveRR_adr);
    ...
    */
	...
	const struct iqs5xx_dev_config *cfg = dev->config;
	//int a = iqs5xx_read_reg(&dev, GestureEvents0_adr, &data, sizeof(data->sample));
	return 0;
